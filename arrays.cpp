...
void task_1()
{
	...
	*  https://gcc.gnu.org/onlinedocs/gcc/Variable-Length.html*/
	int n = 10;
	int a[10]; // Размар массива должен быть константой
...
{
	for (int i = 0; i < size; i++) //т.к. константы нельзя присваивать
		arr1[i] = arr2[i];
}
// Task 6
...
void task_6_poor_copy(int * arr1, int * arr2)
{
	...
	for (int i = 0; i < size(arr2); ++i) //sizeof возвращает колличество битов в массиве
	{
		arr2[i] = arr1[i];
	}
}
...
void task_7_print(int * arr, int size, char delim = ' ')
{
	...
	for (int i = 0; i < size; ++i) // массив индексируется с нуля
	{
		std::cout << a[i] << delim;
	}
}
